...
export class EspnFootballAdapter {
  ...
  async fetchUpcomingMatches(
    ...
    // ESPN scoreboard only shows current matchday by default.
    // To find upcoming matches, we read the calendar from the scoreboard
    // response and query future dates until we have enough matches.
    const data = await this.client.getScoreboard('soccer', espnSlug);
    ...
    for (const date of futureDates) {
      ...
      const dayData = await this.client.getScoreboard('soccer', espnSlug, dateStr);
      const events: any[] = dayData.events ?? [];
      for (const event of events) {
        const match = this.mapEventToMatch(event, competitionId);
